...
function CreateUI (renderer)
  ...
  local toolbar = renderer:addToolBar("Main Toolbar") 

  -- Create Back Button --
  Log("Setting up Purrooser [2/9]")
  local back_button = renderer:addButton(toolbar, "<", function() renderer:back() end) 

  -- Create Forward Button --
  Log("Setting up Purrooser [3/9]")
  local forward_button = renderer:addButton(toolbar, ">", function() renderer:forward() end)

  -- Create Reload Button --
  Log("Setting up Purrooser [4/9]")
  local reload_button = renderer:addButton(toolbar, "↻", function() renderer:reload() end)

  -- Create Home Button --
  Log("Setting up Purrooser [5/9]")
  local home_button = renderer:addButton(toolbar, "⌂", function() renderer:load_local_file() end) 

  -- Create URL Bar --
  Log("Setting up Purrooser [6/9]")
  local url_bar = renderer:addUrlBar(toolbar, function() renderer:navigate_to_url() end)

  -- Create Add Button --
  Log("Setting up Purrooser [7/9]")
  local new_tab_button = renderer:addButton(toolbar, "+", function() renderer:add_new_tab() end)

  -- Create Close Button --
  Log("Setting up Purrooser [8/9]")
  local close_tab_button = renderer:addButton(toolbar, "X", function() renderer:close_current_tab() end)
...
function PrintInfo ()
  local license_url = "https://raw.githubusercontent.com/Thoq-jar/Purrooser/refs/heads/main/LICENSE"
  io.write('\n') -- Make Newline For Cleaner Viewing --
  print("-------------------------------- Information --------------------------------")
  Log("Purrooser")
  Log("v0.1.27 {Nightly}")
  ...
  Log("If you didnt, you can obtain a copy here: " .. license_url)
